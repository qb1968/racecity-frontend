...
import { getCategories, getFilteredProducts4 } from "./apiCore";
...
const Statues = () => {
  ...
  const loadFilteredResults = (newFilters) => {
    ...
    getFilteredProducts4(skip, limit, newFilters).then((data) => {
      if (data.error) {
        setError(data.error);
      } else {
  ...
  const handleFilters = (filters, filterBy) => {
    // console.log("Statues", filters, filterBy);
